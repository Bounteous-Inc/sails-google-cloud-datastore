...
const adapter = {
  ...
  registerDatastore: (dsConfig, models, done) => {
    ...
    if (!keyFilename) {
      let msg = 'Missing required configuration `keyFilename`';
      return done(new Error(msg));
    }

    if (!namespace) {
      let msg = 'Missing required configuration `namespace`';
      return done(new Error(msg));
    }
...
function buildQueries(ds, s3q) {
  ...
  if (hasCriteria) {
    ...
    queries = parseWhereClause(criteria.where).map(filters => {
      ...
      const query = ds.createQuery(namespace, s3q.using);
      ...
      filters.forEach(filter => {
        ...
        const id = (filter[1] + '').match(/^[0-9]+$/) ?
          ds.int(filter[1]) :
          filter[1];
        const key = ds.key({namespace, path: [kind, id]});
    ...
    queries = [ ds.createQuery(namespace, s3q.using) ];
...
function recursiveDelete(config) {
  ...
  const model = _.find(_models, {tableName: using});
  const namespace = model.namespace;
  const keysOnlyQuery = config.keysOnlyQuery || ds.createQuery(namespace, using).select('__key__');
